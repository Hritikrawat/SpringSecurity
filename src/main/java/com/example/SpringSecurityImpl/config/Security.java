...
public class Security
{
    ...
    public SecurityFilterChain securityFilterChain (HttpSecurity request) throws Exception {
        ...
        request.csrf(Customizer->Customizer.disable())
                .authorizeHttpRequests(req->
                        req.requestMatchers("/user/register","/user/login").permitAll().anyRequest().authenticated())
    ...
    //AunthenticationManager Calls AuthenticationPoviders (internally aunthentication manger hanldes everything)
    @Bean
    public AuthenticationProvider authenticationProvider() {
        DaoAuthenticationProvider authenticator = new DaoAuthenticationProvider();

        authenticator.setPasswordEncoder(new BCryptPasswordEncoder(10));
